...
async function GET(request) {
    try {
        ...
        const popularRecipes = await db.collection('recipes').find({
            isPublished: true
        }).sort({
            views: -1
        }).limit(3).toArray();
        // Utilisateurs récents
        const recentUsers = await db.collection('users').find({}).sort({
            createdAt: -1
        }).limit(3).toArray();
        // Recettes récentes
        const recentRecipes = await db.collection('recipes').find({
            isPublished: true
        }).sort({
            createdAt: -1
        }).limit(3).toArray();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1cb55d01._.js.map